...
$WORDPRESS_PIXELPIN_LOGIN_VERSION = "2.3.0";

$_SESSION["wsl::plugin"] = "WordPress PixelPin Login " . $WORDPRESS_PIXELPIN_LOGIN_VERSION;
...
function wsl_add_plugin_action_links( $links, $file )
{
	...
	if( $file == $this_plugin )
	{
		$wsl_links  = '<a href="options-general.php?page=wordpress-pixelpin-login">' . __( "Settings" ) . '</a>';
...
# WSL Setup & Settings
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/settings/wsl.providers.php'            ); // List of supported providers (mostly provided by hybridauth library)
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/settings/wsl.database.php'             ); // Install/Uninstall WSL database tables
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/settings/wsl.initialization.php'       ); // Check WSL requirements and register WSL settings
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/settings/wsl.compatibilities.php'      ); // Check and upgrade WSL database/settings (for older versions)

# Services & Utilities
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/services/wsl.authentication.php'       ); // Authenticate users via pixelpin networks. <- that's the most important script
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/services/wsl.mail.notification.php'    ); // Emails and notifications
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/services/wsl.user.avatar.php'          ); // Display users avatar
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/services/wsl.user.data.php'            ); // User data functions (database related)
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/services/wsl.utilities.php'            ); // Unclassified functions & utilities
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/services/wsl.watchdog.php'             ); // WSL logging agent

# WSL Widgets & Front-end interfaces
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/widgets/wsl.auth.widgets.php'          ); // Authentication widget generators (where WSL widget/icons are displayed)
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/widgets/wsl.users.gateway.php'         ); // Accounts linking + Profile Completion
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/widgets/wsl.error.pages.php'           ); // Generate WSL notices end errors pages
require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/widgets/wsl.loading.screens.php'       ); // Generate WSL loading screens

# WSL Admin interfaces
if( is_admin() && ( !defined( 'DOING_AJAX' ) || !DOING_AJAX ) )
{
	require_once( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . 'includes/admin/wsl.admin.ui.php'        ); // The entry point to WSL Admin interfaces
}
