...
function wsl_process_login_new_users_gateway( $provider, $redirect_to, $hybridauth_user_profile )
{
	...
	if( isset( $_REQUEST["bouncer_account_linking"] ) )
	{
		...
		{
			return wsl_process_login_render_notice_page( _wsl__( "Not tonight.", 'wordpress-pixelpin-login' ) );
		}
		...
		else
		{
			...
			*/
			...
			if( $require_email == 1 )
			{
				...
				{
					$profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: Please type your e-mail address.', 'wordpress-pixelpin-login' );
				}

				if ( ! is_email( $requested_user_email ) )
				{
					$profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: Please enter a valid email address.', 'wordpress-pixelpin-login' );
				}

				if ( wsl_wp_email_exists( $requested_user_email ) )
				{
					$profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: Sorry, that email address is already used!', 'wordpress-pixelpin-login' );
				}
			}
			# }}} validate usermail
			...
			if( $change_username == 1 )
			{
				...
				{
					$profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: That username is not allowed.', 'wordpress-pixelpin-login' );
				}

				if ( strlen( $requested_user_login ) < 4 )
				{
					$profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: Username must be at least 4 characters.', 'wordpress-pixelpin-login' );
				}

				if ( strpos( ' ' . $requested_user_login, '_' ) != false )
				{
					// $profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: Sorry, usernames may not contain the character &#8220;_&#8221;!', 'wordpress-pixelpin-login' );
				}

				if ( preg_match( '/^[0-9]*$/', $requested_user_login ) )
				{
					$profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: Sorry, usernames must have letters too!', 'wordpress-pixelpin-login' );
				}

				if ( username_exists( $requested_user_login) )
				{
					$profile_completion_errors[] = _wsl__( '<strong>ERROR</strong>: Sorry, that username already exists!', 'wordpress-pixelpin-login' );
				}
			}
			# }}} validate username
		...
		<div id="login">
			<div id="login-panel">
				...
				<div id="welcome">
					...
					<b><?php printf( _wsl__( "Hi %s", 'wordpress-pixelpin-login' ), htmlentities( $hybridauth_user_profile->displayName ) ); ?></b>
					<p><?php printf( _wsl__( "You're now signed in with your %s account but you are still one step away of getting into our website", 'wordpress-pixelpin-login' ), $provider ); ?>.</p>
				...
				<table id="mapping-options" border="0">
					<tr>
						<?php if( $linking_enabled == 1 ): ?>
							<td valign="top"  width="50%" style="text-align:center;">
								<h4><?php _wsl_e( "Already have an account", 'wordpress-pixelpin-login' ); ?>?</h4>
								<p style="font-size: 12px;"><?php printf( _wsl__( "Link your existing account on our website to your %s ID.", 'wordpress-pixelpin-login' ), $provider ); ?></p>
							</td>
						<?php endif; ?>

						<td valign="top"  width="50%" style="text-align:center;">
							<h4><?php _wsl_e( "New to our website", 'wordpress-pixelpin-login' ); ?>?</h4>
							<p style="font-size: 12px;"><?php printf( _wsl__( "Create a new account and it will be associated with your %s ID.", 'wordpress-pixelpin-login' ), $provider ); ?></p>
						</td>
					</tr>
					...
					<tr>
						<?php if( $linking_enabled == 1 ): ?>
							<td valign="top"  width="50%" style="text-align:center;">
								<input type="button" value="<?php _wsl_e( "Link my account", 'wordpress-pixelpin-login' ); ?>" class="button-primary" onclick="display_mapping_authenticate();" >
							</td>
						<?php endif; ?>

						<td valign="top"  width="50%" style="text-align:center;">
							<?php if( $require_email != 1 && $change_username != 1 && $extra_fields != 1 ): ?>
                                <input type="button" value="<?php _wsl_e( "Create a new account", 'wordpress-pixelpin-login' ); ?>" class="button-primary" onclick="document.getElementById('info-form').submit();" >
							<?php else : ?>
                                <input type="button" value="<?php _wsl_e( "Create a new account", 'wordpress-pixelpin-login' ); ?>" class="button-primary" onclick="display_mapping_complete_info();" >
                            <?php endif; ?>
						</td>
					</tr>
				</table>
					...
					<table id="mapping-authenticate" border="0">
						<tr>
							<td valign="top"  width="50%" style="text-align:center;">
								...
								<p><?php printf( _wsl__( "Please enter your username and password of your existing account on our website. Once verified, it will linked to your % ID", 'wordpress-pixelpin-login' ), ucfirst( $provider ) ) ; ?>.</p>
							</td>
						</tr>
						<tr>
							<td valign="bottom"  width="50%" style="text-align:left;">
								<label>
									<?php _wsl_e( "Username", 'wordpress-pixelpin-login' ); ?>
									<br />
									<input type="text" name="user_login" class="input" value=""  size="25" placeholder="" />
								</label>

								<label>
									<?php _wsl_e( "Password", 'wordpress-pixelpin-login' ); ?>
									<br />
									<input type="text" name="user_password" class="input" value="" size="25" placeholder="" />
								</label>

								<input type="submit" value="<?php _wsl_e( "Continue", 'wordpress-pixelpin-login' ); ?>" class="button-primary" >

								<a href="javascript:void(0);" onclick="display_mapping_options();" class="back-to-options"><?php _wsl_e( "Back", 'wordpress-pixelpin-login' ); ?></a>
							</td>
						</tr>
					</table>
					...
					<table id="mapping-complete-info" border="0">
						<tr>
							<td valign="top"  width="50%" style="text-align:center;">
								...
								<p><?php printf( _wsl__( "Please fill in your information in the form below. Once completed, you will be able to automatically sign into our website through your %s ID", 'wordpress-pixelpin-login' ), $provider_name ); ?>.</p>
							</td>
						</tr>
						<tr>
							<td valign="bottom"  width="50%" style="text-align:left;">
                                ...
                                <?php if( $require_email == 1 ): ?>
                                    <label>
                                        <?php _wsl_e( "E-mail", 'wordpress-pixelpin-login' ); ?>
                                        <br />
			...
			<p class="back-to-home">
				<a href="<?php echo home_url(); ?>">&#8592; <?php printf( _wsl__( "Back to %s", 'wordpress-pixelpin-login' ), get_bloginfo('name') ); ?></a>
			</p>
		</div>
