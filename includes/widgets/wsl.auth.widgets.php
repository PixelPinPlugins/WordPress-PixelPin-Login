...
function wsl_render_auth_widget( $args = array() )
{
	...
	$pixelpin_icon_set = get_option( 'wsl_settings_pixelpin_icon_set' );
	...
	// build the authentication url which will call for wsl_process_login() : action=wordpress_pixelpin_authenticate
	...
	$connect_with_label = _wsl__( get_option( 'wsl_settings_connect_with_label' ), 'wordpress-pixelpin-login' );
...
<!--
	...
	WordPress PixelPin Login <?php echo wsl_get_version(); ?>.
...
?>
		...
		<a rel="nofollow" href="<?php echo $authenticate_url; ?>" title="<?php echo sprintf( _wsl__("Connect with %s", 'wordpress-pixelpin-login'), $provider_name ) ?>" class="wp-pixelpin-login-provider wp-pixelpin-login-provider-<?php echo strtolower( $provider_id ); ?>" data-provider="<?php echo $provider_id ?>">
			<?php if( $pixelpin_icon_set == 'none' ){ echo apply_filters( 'wsl_render_auth_widget_alter_provider_name', $provider_name ); } else { ?><img alt="<?php echo $provider_name ?>" title="<?php echo sprintf( _wsl__("Connect with %s", 'wordpress-pixelpin-login'), $provider_name ) ?>" src="<?php echo $assets_base_url . strtolower( $provider_id ) . '.png' ?>" /><?php } ?>
...
*/
function wsl_action_wordpress_pixelpin_login( $args = array() )
{
	echo wsl_render_auth_widget( $args );
}

add_action( 'wordpress_pixelpin_login', 'wsl_action_wordpress_pixelpin_login' );
...
*/
function wsl_shortcode_wordpress_pixelpin_login( $args = array(), $content = null )
{
...
add_shortcode( 'wordpress_pixelpin_login', 'wsl_shortcode_wordpress_pixelpin_login' );
...
*   ]
*/
function wsl_shortcode_wordpress_pixelpin_login_meta( $args = array() )
{
	...
	if( 'current_avatar' == $meta )
	{
		...
		{
			$return = '<img class="wordpress_pixelpin_login_meta_user_avatar ' . $css_class . '" src="' . wsl_get_user_custom_avatar( $user_id ) . '" />';
		}
	}
...
add_shortcode( 'wordpress_pixelpin_login_meta', 'wsl_shortcode_wordpress_pixelpin_login_meta' );
...
function wsl_add_stylesheets()
{
	...
	{
		wp_register_style( "wsl-widget", WORDPRESS_PIXELPIN_LOGIN_PLUGIN_URL . "assets/css/style.css" );
	}
...
function wsl_add_javascripts()
{
	...
	{
		wp_register_script( "wsl-widget", WORDPRESS_PIXELPIN_LOGIN_PLUGIN_URL . "assets/js/widget.js" );
	}
