...
*     .       .       .       .       wsl_process_login_request_user_pixelpin_profile()
...
function wsl_process_login()
{
	...
	if( 'login' == $auth_mode && is_user_logged_in() )
	{
		...
		return wsl_process_login_render_notice_page( sprintf( _wsl__( "You are already logged in as %s. Do you want to <a href='%s'>log out</a>?", 'wordpress-pixelpin-login' ), $current_user->display_name, wp_logout_url( home_url() ) ) );
	}

	// halt, if mode link and user not logged in
	if( 'link' == $auth_mode && ! is_user_logged_in() )
	{
		return wsl_process_login_render_notice_page( sprintf( _wsl__( "You have to be logged in to be able to link your existing account. Do you want to <a href='%s'>login</a>?", 'wordpress-pixelpin-login' ), wp_login_url( home_url() ) ) );
	}

	// halt, if mode test and not admin
	if( 'test' == $auth_mode && ! current_user_can('manage_options') )
	{
		return wsl_process_login_render_notice_page( _wsl__( 'You do not have sufficient permissions to access this page.', 'wordpress-pixelpin-login' ) );
	}

	// Bouncer :: Allow authentication?
	if( get_option( 'wsl_settings_bouncer_authentication_enabled' ) == 2 )
	{
		return wsl_process_login_render_notice_page( _wsl__( "Authentication through pixelpin networks is currently disabled.", 'wordpress-pixelpin-login' ) );
	}
...
function wsl_process_login_begin()
{
	...
	{
		return wsl_process_login_render_notice_page( sprintf( _wsl__( 'The session identifier is missing.<br />For more information refer to WSL <a href="http://miled.github.io/wordpress-pixelpin-login/troubleshooting.html#session-error" target="_blank">Troubleshooting</a>.', 'wordpress-pixelpin-login' ), home_url() ) );
	}
	...
	if( ! $provider )
	{
		return wsl_process_login_render_notice_page( _wsl__( 'Bouncer says this makes no sense.', 'wordpress-pixelpin-login' ) );
	}
	...
	{
		return wsl_process_login_render_notice_page( _wsl__( "Unknown or disabled provider.", 'wordpress-pixelpin-login' ) );
	}
...
function wsl_process_login_end()
{
	...
	if( 'test' == $auth_mode )
	{
		$redirect_to = admin_url( 'options-general.php?page=wordpress-pixelpin-login&wslp=auth-paly&provider=' . $provider );
	...
	if( 'link' == $auth_mode )
	{
		...
		$hybridauth_user_profile = wsl_process_login_request_user_pixelpin_profile( $provider );
		...
		{
			return wsl_process_login_render_notice_page( sprintf( _wsl__( "Your <b>%s ID</b> is already linked to another account on this website.", 'wordpress-pixelpin-login'), $provider ) );
		}
		...
		if( ! $user_id )
		{
			return wsl_process_login_render_notice_page( _wsl__( "Sorry, we couldn't link your account.", 'wordpress-pixelpin-login' ) );
		}
	}
	elseif( 'login' != $auth_mode )
	{
		return wsl_process_login_render_notice_page( _wsl__( 'Bouncer says no.', 'wordpress-pixelpin-login' ) );
	}
	...
	if( ! $wp_user )
	{
		...
		return wsl_process_login_render_notice_page( sprintf( _wsl__( "Sorry, we couldn't connect you. <a href=\"%s\">Please try again</a>.", 'wordpress-pixelpin-login' ), site_url( 'wp-login.php', 'login_post' ) ) );
	}
...
function wsl_process_login_get_user_data( $provider, $redirect_to )
{
	...
	if( get_option( 'wsl_settings_bouncer_new_users_restrict_domain_enabled' ) == 1 )
	{
		...
		{
			return wsl_process_login_render_notice_page( _wsl__( get_option( 'wsl_settings_bouncer_new_users_restrict_domain_text_bounce' ), 'wordpress-pixelpin-login') );
		}
	...
	// the user BEFORE we filter by email address
        ...
        if( ! $user_id )
        {
                ...
                {
                        return wsl_process_login_render_notice_page( _wsl__( "Registration is now closed.", 'wordpress-pixelpin-login' ) );
                }
	...
	if( get_option( 'wsl_settings_bouncer_new_users_restrict_email_enabled' ) == 1 )
	{
		...
		if( empty( $hybridauth_user_email ) )
		{
			return wsl_process_login_render_notice_page( _wsl__( get_option( 'wsl_settings_bouncer_new_users_restrict_email_text_bounce' ), 'wordpress-pixelpin-login') );
		}
	...
	if( get_option( 'wsl_settings_bouncer_new_users_restrict_profile_enabled' ) == 1 )
	{
		...
		if( ! $shall_pass )
		{
			return wsl_process_login_render_notice_page( _wsl__( get_option( 'wsl_settings_bouncer_new_users_restrict_profile_text_bounce' ), 'wordpress-pixelpin-login') );
		}
	}
...
function wsl_process_login_create_wp_user( $provider, $hybridauth_user_profile, $requested_user_login, $requested_user_email )
{
	...
	if( ! $user_id || ! is_integer( $user_id ) )
	{
		...
		{
			return wsl_process_login_render_notice_page( _wsl__( "An error occurred while creating a new user: ", 'wordpress-pixelpin-login' ) . $user_id->get_error_message() );
		}

		return wsl_process_login_render_notice_page( _wsl__( "An error occurred while creating a new user!", 'wordpress-pixelpin-login' ) );
	}
...
*/
function wsl_process_login_request_user_pixelpin_profile( $provider )
{
	$adapter                 = null;
	$config                  = null;
	$hybridauth_user_profile = null;
	...
	try
	{
		...
		{
			return wsl_process_login_render_notice_page( sprintf( _wsl__( "Sorry, we couldn't connect you with <b>%s</b>. <a href=\"%s\">Please try again</a>.", 'wordpress-pixelpin-login' ), $provider, site_url( 'wp-login.php', 'login_post' ) ) );
		}
	}
...
function wsl_process_login_render_error_page( $e, $config = null, $provider = null, $adapter = null )
{
	...
	$message  = _wsl__("Unspecified error!", 'wordpress-pixelpin-login');
	$notes    = "";
	$apierror = substr( $e->getMessage(), 0, 145 );

	switch( $e->getCode() )
	{
		case 0 : $message = _wsl__("Unspecified error.", 'wordpress-pixelpin-login'); break;
		case 1 : $message = _wsl__("WordPress PixelPin Login is not properly configured.", 'wordpress-pixelpin-login'); break;
		case 2 : $message = sprintf( __wsl__("WordPress PixelPin Login is not properly configured.<br /> <b>%s</b> need to be properly configured.", 'wordpress-pixelpin-login'), $provider ); break;
		case 3 : $message = _wsl__("Unknown or disabled provider.", 'wordpress-pixelpin-login'); break;
		case 4 : $message = sprintf( _wsl__("WordPress PixelPin Login is not properly configured.<br /> <b>%s</b> requires your application credentials.", 'wordpress-pixelpin-login'), $provider );
			 $notes   = sprintf( _wsl__("<b>What does this error mean ?</b><br />Most likely, you didn't setup the correct application credentials for this provider. These credentials are required in order for <b>%s</b> users to access your website and for WordPress PixelPin Login to work.", 'wordpress-pixelpin-login'), $provider ) . _wsl__('<br />Instructions for use can be found in the <a href="http://miled.github.io/wordpress-pixelpin-login/networks.html" target="_blank">User Manual</a>.', 'wordpress-pixelpin-login');
			 break;
		case 5 : $message = sprintf( _wsl__("Authentication failed. Either you have cancelled the authentication or <b>%s</b> refused the connection.", 'wordpress-pixelpin-login'), $provider ); break;
		case 6 : $message = sprintf( _wsl__("Request failed. Either you have cancelled the authentication or <b>%s</b> refused the connection.", 'wordpress-pixelpin-login'), $provider ); break;
		case 7 : $message = _wsl__("You're not connected to the provider.", 'wordpress-pixelpin-login'); break;
		case 8 : $message = _wsl__("Provider does not support this feature.", 'wordpress-pixelpin-login'); break;
	}
...
function wsl_process_login_clear_user_php_session()
{
	...
	$_SESSION["wsl::userprofile"] = array(); // used by wsl to temporarily store the user profile so we don't make unnecessary calls to pixelpin apis.
}
