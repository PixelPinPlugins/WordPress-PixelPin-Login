...
<form action="" method="post">
	<table class="widefat fixed plugins" cellspacing="0">
		<thead>
			<tr>
				<th scope="col" class="manage-column column-label" style="width: 190px;"><?php _wsl_e( "Component", 'wordpress-pixelpin-login' ) ?></th>
				<th scope="col" class="manage-column column-description"><?php _wsl_e( "Description", 'wordpress-pixelpin-login' ) ?></th>
			...
			?>
				<tr id="<?php echo $name ?>" class="<?php echo $name ?> <?php echo $plugin_tr_class ?>"> 
					<td class="component-label" style="width: 190px;"> &nbsp;
						...
						<strong><?php _wsl_e( $settings["label"], 'wordpress-pixelpin-login' ) ?></strong> 
					</td>
					<td class="column-description">
						<p><?php _wsl_e( $settings["description"], 'wordpress-pixelpin-login' ) ?></p>
						<?php
							$meta = array();

							if( isset( $settings["version"] ) )
							{
								$meta[] = sprintf( _wsl__( "Version %s", 'wordpress-pixelpin-login' ), $settings["version"] );
							}
							...
							if( isset( $settings["author"] ) )
							{
								...
								{
									$meta[] = sprintf( _wsl__( 'By <a href="%s" target="_blank">%s</a>', 'wordpress-pixelpin-login' ), $settings["author_url"], $settings["author"] );
								}
								else
								{
									$meta[] = sprintf( _wsl__( 'By %s', 'wordpress-pixelpin-login' ), $settings["author"] );
								}
							}

							if( isset( $settings["component_url"] ) )
							{
								$meta[] = sprintf( _wsl__( '<a href="%s" target="_blank">Visit component site</a>', 'wordpress-pixelpin-login' ), $settings["component_url"] );
							}
						...
						<p>
							<?php if( $plugin_enablable && $settings["type"] != "core" ): ?>
								<?php if( $settings["enabled"] ): ?> 
									<a class="button-secondary" href="options-general.php?page=wordpress-pixelpin-login&wslp=components&disable=<?php echo $name ?>"><?php _wsl_e( "Disable", 'wordpress-pixelpin-login' ) ?></a>
								<?php else: ?>
									<a class="button-primary" style="color:#ffffff" href="options-general.php?page=wordpress-pixelpin-login&wslp=components&enable=<?php echo $name ?>"><?php _wsl_e( "Enable", 'wordpress-pixelpin-login' ) ?>&nbsp;</a>
								<?php endif; ?>
							<?php endif; ?>
							&nbsp;
						</p>
